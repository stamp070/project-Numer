...
const Cramers = () => {
  ...
  const random = () => {
    axios.get(
      `${import.meta.env.VITE_server_port}/load/linearalgebra/all`, 
      {
        headers: {
    ...
    .then(res => {
      const dataA = res.data.equationsA[0]; 
      const dataB = res.data.equationsB[0]; 
      
      setMatrixB(dataA.equationA);
      setMatrixA(dataB.equationB);
      
      setEquation(randomEquation); // Update state with the selected equation
    })
    .catch(error => {
